...
describe("Series", () => {
  ...
  describe("clone()", () => {
    ...
    test("it should return overwrited instance", () => {
      ...
      expect(changedType.interval.asSeconds()).toBe(
        original.interval.asSeconds()
      );
      ...
      expect(changedFrom.interval.asSeconds()).toBe(
        original.interval.asSeconds()
      );
      ...
      expect(changedUntil.interval.asSeconds()).toBe(
        original.interval.asSeconds()
      );
      ...
      expect(changedNumOfData.interval.asSeconds()).toBe(
        original.interval.asSeconds()
      );
      ...
      expect(changedKeyName.interval.asSeconds()).toBe(
        original.interval.asSeconds()
      );
  ...
  describe("_timestamps()", () => {
    test("it should return unix timestamp series", () => {
      ...
      const seriesMonospaced = new Series({
        type: "monospaced",
        from,
        until,
        interval
      ...
      jsc.assertForall(jsc.unit, () => {
        ...
        expect(
          outputRandom.every(unix => 1451606400 <= unix && unix <= 1451610000)
        ).toBeTruthy();
    ...
    test("it should return unix timestamp series by default options", () => {
      ...
      jsc.assertForall(jsc.unit, () => {
        ...
        expect(
          outputRandom.every(unix => 1495753200 <= unix && unix <= 1495756800)
        ).toBeTruthy();
        expect(outputRandom).toEqual(outputRandom.sort());
    ...
    describe("sin()", () => {
      ...
      test("it should return sine curve", () => {
        ...
        jsc.assertForall(jsc.unit, () => {
          ...
          expect(
            outputRandom.every(({ timestamp, value }) => {
              return (
                "2016-01-01T00:00:00.000Z" <= timestamp &&
                timestamp <= "2016-01-01T01:00:00.000Z" &&
                (-1 <= value && value <= 3)
              );
            })
          ).toBeTruthy();
  ...
  describe("gaussian()", () => {
    ...
    test("it should return time series data with 1", () => {
      ...
      jsc.assertForall(jsc.unit, () => {
        ...
        expect(
          outputMonospaced.every(({ timestamp, value }) => {
            return (
              "2016-01-01T00:00:00.000Z" <= timestamp &&
              timestamp <= "2016-01-01T01:00:00.000Z" &&
              value === 1
    ...
    test("it should return time series numbers with gaussian distribution", () => {
      ...
      jsc.assertForall(jsc.unit, () => {
        ...
        expect(outputMonospaced.length).toBe(7);
        expect(
          outputMonospaced.every(({ timestamp }) => {
            return (
              "2016-01-01T00:00:00.000Z" <= timestamp &&
              timestamp <= "2016-01-01T01:00:00.000Z"
            );
