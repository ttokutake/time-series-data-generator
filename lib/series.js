...
class Series {
  ...
  ratio(params) {
    ...
    return this._timestamps().map((unix) => this._toElem(unix, ratio.sample()));
  }

  _timestamps() {
    switch (this.type) {
      case TYPE_MONOSPACED:
        return Range(this.from.unix(), this.until.unix() + 1, this.interval.asSeconds()).toJSON();
      case TYPE_RANDOM:
        const min = this.from .unix();
        const max = this.until.unix();
        return R.pipe(R.map((x) => randomInt(min, max)), R.sort((x, y) => x - y))(R.range(0, this.numOfData));
      default:
        assert(false);
    }
  }
  ...
  _toElem(unix, value) {
    return R.assoc(this.keyName, value, {timestamp: moment(unix * 1000).toISOString()});
  }
}
