...
class Series {
  ...
  _trigonometric(func, options = {}) {
    ...
    return this._range()
      .reduce((map, t) => map.set(t, func(t * scale)), OrderedMap())
      .map((v) => options.coefficient * v + options.constant)
      .map((v) => math.round(v, 2))
      .map((v, t) => this._toElem(t, options.keyName, v))
      .toList()
      .toJS();
  }
  ...
  ratio(params, options = {}) {
    ...
    return this._range()
      .map((t) => this._toElem(t, options.keyName, ratio.sample()))
      .toJS();
  }
  ...
  _toElem(unix, keyName, value) {
    return Map({timestamp: moment(unix * 1000).toISOString()}).set(keyName, value);
  }
}
