...
function dummyAccessLog({numOfLines, numOfUsers, keysOfCrudReq, dateRange}) {
  ...
  const users = Range(0, numOfUsers)
    .map((_) => {
      return {email: faker.internet.email(), uuid: faker.random.uuid()};
    })
  ...
  const lines = Range(0, numOfLines)
    .map((_) => {
      ...
      const request       = getRandomCrudRequest(key, key === 'users' ? uuid : faker.random.uuid());
      return Map({user: email, timestamp: date.toISOString()}).merge(request);
    });
