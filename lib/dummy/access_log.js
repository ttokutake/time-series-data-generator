...
function dummyAccessLog({numOfLines, numOfUsers, keysOfCrudReq}) {
  ...
  if (isNotArray(keysOfCrudReq) || keysOfCrudReq.some(isNotString)) {
    throw new Error('1st argument{keysOfCrudReq} must be "undefined", "null" or array of "String"');
  }
  ...
  const lines = Range(0, numOfLines)
    .map((_) => {
      ...
      return Map({user: email, date: F.date.recent().toISOString()}).merge(request);
    });
